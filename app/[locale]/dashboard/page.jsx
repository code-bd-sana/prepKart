...
export default function DashboardPage() {
  ...
  const formatDate = (dateString) => {
    ...
    return date.toLocaleDateString(locale === 'fr' ? 'fr-CA' : 'en-CA', options);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">{t('accountOverview')}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-gray-500">{t('emailAddress')}</p>
                    <p className="font-medium truncate">{user?.email || t('notAvailable')}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">{t('location')}</p>
                    <p className="font-medium">{user?.province || t('notSet')}</p>
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`font-medium px-3 py-1 rounded-full text-sm ${config.color}`}>
                        {config.name} {t('yourPlan').split(' ')[1]} {/* Shows "Plan" */}
                      </span>
                  ...
                  <div>
                    <p className="text-sm text-gray-500">{t('swapsAvailable')}</p>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-lg">
                        {user?.swapsUsed || 0} / {user?.swapsAllowed || config.swapsAllowed} {t('used')}
                      </span>
          ...
          <div className="space-y-6">
            {/* Plan Status Card */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">{t('yourPlan')}</h2>
              
              <div className={`p-4 rounded-lg ${config.color} mb-4`}>
                <div className="font-bold text-lg">{config.name} {t('yourPlan').split(' ')[1]}</div>
                <div className="text-sm opacity-90">{config.price}</div>
              </div>

              <div className="space-y-3">
                <Link
                  href={`/${locale}/#pricing`} // Add locale
                  className="block w-full text-center bg-[#8cc63c] hover:bg-[#7ab32f] text-white py-3 rounded-lg font-medium"
                >
                  {currentTier === 'free' ? t('upgradePlan') : t('manageSubscription')}
                </Link>

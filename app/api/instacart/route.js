...
//     console.log('Received items:', groceryItems.length);
...
//     const INSTACART_API_ENDPOINT = process.env.INSTACART_API_ENDPOINT || "https://connect.dev.instacart.tools";
    
//     // Debug: Check if API key is loaded
//     console.log('API Key loaded:', !!INSTACART_API_KEY);
//     console.log('API Key starts with "keys.":', INSTACART_API_KEY?.startsWith('keys.'));
//     console.log('API Key length:', INSTACART_API_KEY?.length);
...
//     // Format items EXACTLY like Postman example
//     const line_items = checkedItems.map(item => {
//       // Convert unit to proper format
//       const unit = mapToInstacartUnit(item.unit || 'unit');
      
//       // Quantity MUST be string (as per Postman example)
...
//           {
//             quantity: quantity,  // String, not number
//             unit: unit
//           }
//         ]
//       };
...
//     console.log('Endpoint:', `${INSTACART_API_ENDPOINT}/idp/v1/products/products_link`);
//     console.log('Payload sample:', JSON.stringify(payload.line_items[0], null, 2));
    
//     // Make the request
//     const response = await fetch(`${INSTACART_API_ENDPOINT}/idp/v1/products/products_link`, {
//       method: "POST",
//       headers: {
...
async function trackInstacartClick(trackingData) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/clicks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        ...
        checkedItemsCount: trackingData.checkedItemsCount || trackingData.totalItems || 0,
        metadata: {
    ...
    console.log('✅ Click tracked via API:', result.success ? 'Success' : 'Failed');
    return result;
...
export async function POST(request) {
  ...
  try {
    ...
    console.log('Tracking result:', trackingResult.success ? '✅ Tracked' : '❌ Failed');

...
export async function generateSpoonacularMealPlan(inputs, userTier = 'tier2') {
  try {
    ...
    // Step 1: Build SMART search parameters
    const searchParams = buildSmartSearchParams(inputs);
    
    // Step 2: Search for recipes with multiple attempts
...
function buildSmartSearchParams(inputs) {
  console.log('Building SMART search params...');
  
  // Build a BETTER query
  const queryParts = [];
  
  // 1. Add protein preferences from likes
  if (inputs.likes) {
  ...
  // 2. Add cuisine as keyword (not filter)

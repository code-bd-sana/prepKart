...
`;
    ...
    if (planData.days.length === 0) {
      console.log("⚠️ AI returned empty days array, creating fallback structure");
    ...
    console.log(
      `✅ Plan generated: ${planData.days.length} days, ${
        planData.days[0]?.meals?.length || 0
      } meals/day`
    );
...
export async function generateAlternativeMeal(
  ...
  console.log("=== ALTERNATIVE MEAL GENERATION ===");
  console.log("Generating alternative for:", oldMealName);
  console.log("User Tier for swap:", userTier);

  const actualTier = userTier || inputs.userTier || "free";
  console.log("Actual user tier:", actualTier);
...
Return ONLY JSON:
{
  ...
  "instructions": ["Step 1","Step 2"]
    ...
    console.log(`Sending alternative request to OpenAI`);
    ...
    console.log("✅ Alternative OpenAI response received");
    ...
    console.log(`✅ Alternative generated for ${actualTier} user:`, completeMeal.recipeName);
    console.log(`🔄 Changed from "${oldMealName}" to "${completeMeal.recipeName}"`);
    ...
    console.log(`⚠️ Using fallback alternative: ${recipeName} (was: ${oldMealName})`);
    return fallbackMeal;
  }
}
...
export async function generateMealPlan(inputs, userTier = "free") {
  ...
  try {
    ...
    if (dietaryPreferences.includes("vegan")) {
      ...
      adjustedDislikes = (
        adjustedDislikes +
        ", eggs, dairy, meat, fish, poultry, seafood, honey"
      ).replace(/^,\s*/, "");
    }
    ...
    if (mealsPerDay === 1) {
      ...
      mealTypes = ["breakfast", "mid-morning snack", "lunch", "afternoon snack", "dinner"];
      if (mealsPerDay > 5) {
...
5. All recipes MUST comply with: ${dietaryPreferences.join(", ") || "no restrictions"}
...
10. Meals must be appropriate for their type (breakfast, lunch, dinner, snack)
...
Return ONLY the JSON object.`,
    ...
    if (planData.days.length !== daysCount) {
      console.log(`Adjusting days from ${planData.days.length} to ${daysCount}`);
      if (planData.days.length > daysCount) {
    ...
    planData.days.forEach((day, dayIndex) => {
      ...
      if (day.meals.length !== mealsPerDay) {
        console.log(`Day ${dayIndex + 1}: Adjusting meals from ${day.meals.length} to ${mealsPerDay}`);
...
function createEnhancedFallbackMeal(mealType, inputs) {
  ...
  const mealTemplates = {
    breakfast: {
      vegan: ["Tofu Scramble with Veggies", "Oatmeal with Berries", "Avocado Toast"],
      vegetarian: ["Vegetable Omelette", "Greek Yogurt Parfait", "Cheese Toastie"],
  ...
  const templates = mealTemplates[mealType.toLowerCase()] || mealTemplates.snack;
  const recipeNames = templates[category] || templates.regular || templates.vegan;
  
  const recipeName = `${cuisine !== "any" ? cuisine + " " : ""}${recipeNames[Math.floor(Math.random() * recipeNames.length)]}`;
...
function createDefaultIngredients(mealType, inputs) {
  ...
  const isVegetarian = inputs.dietaryPreferences?.includes("vegetarian") || false;
  const portions = inputs.portions || 2;

  const baseIngredients = [
    { name: "Salt", quantity: 0.5, unit: "teaspoon", notes: "to taste" },
    { name: "Black pepper", quantity: 0.25, unit: "teaspoon", notes: "freshly ground" },
    { name: "Olive oil", quantity: 1, unit: "tablespoon", notes: "or cooking oil of choice" }
  ];
  ...
  switch (mealType.toLowerCase()) {
    case "breakfast":
      if (isVegan) {
        mealSpecificIngredients = [
          { name: "Firm tofu", quantity: portions * 0.5, unit: "block", notes: "crumbled" },
          { name: "Bell peppers", quantity: portions * 0.5, unit: "cup", notes: "diced" },
          { name: "Onion", quantity: portions * 0.25, unit: "cup", notes: "chopped" },
          { name: "Spinach", quantity: portions * 1, unit: "cup", notes: "fresh" }
        ];
      } else if (isVegetarian) {
        mealSpecificIngredients = [
          { name: "Eggs", quantity: portions * 2, unit: "large" },
          { name: "Milk", quantity: portions * 2, unit: "tablespoons" },
          { name: "Cheese", quantity: portions * 0.25, unit: "cup", notes: "shredded" },
          { name: "Tomatoes", quantity: portions * 0.5, unit: "cup", notes: "diced" }
        ];
      } else {
        mealSpecificIngredients = [
          { name: "Eggs", quantity: portions * 2, unit: "large" },
    ...
    case "lunch":
      mealSpecificIngredients = [
        ...
        { name: "Cucumber", quantity: portions * 0.5, unit: "cup", notes: "sliced" }
      ];
      
      if (!isVegan) {
        mealSpecificIngredients.push(
          { name: "Protein of choice", quantity: portions * 1, unit: "serving" }
        );
      }
      break;
    ...
    case "dinner":
      if (isVegan) {
        mealSpecificIngredients = [
          { name: "Lentils", quantity: portions * 0.5, unit: "cup", notes: "dry" },
          { name: "Carrots", quantity: portions * 1, unit: "cup", notes: "chopped" },
          { name: "Potatoes", quantity: portions * 1, unit: "cup", notes: "cubed" }
        ];
      } else {
        mealSpecificIngredients = [
          { name: "Protein (chicken/fish/tofu)", quantity: portions * 1, unit: "serving" },
          { name: "Rice", quantity: portions * 0.5, unit: "cup", notes: "uncooked" },
...
function generateSimpleFallbackPlan(inputs) {
  ...
  for (let i = 0; i < daysCount; i++) {
    const meals = [];
    const mealTypes = mealsPerDay === 1 ? ["lunch"] : 
...
// Keep other functions (generateAlternativeMeal, generateChatGPTRecipe, etc.) as they are
...
Return ONLY JSON:
{
  ...
  "description": "Brief description"
    ...
    const completeMeal = {
      mealType: validatedMealType,
      recipeName: mealData.recipeName || `Alternative ${cuisine} ${validatedMealType}`,
      description: mealData.description || `A delicious alternative ${validatedMealType}`,
...
function getFallbackRecipe(prompt) {
  ...
  const mealMatch = prompt.toLowerCase().match(/create a (\w+) recipe/i) || 
  ...
  return {
    title: `${cuisine ? cuisine + " " : ""}${mealType.charAt(0).toUpperCase() + mealType.slice(1)}`,
    mealType: mealType,

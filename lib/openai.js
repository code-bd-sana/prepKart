...
try {
    ...
    if (dietaryPreferences.includes('vegan')) {
      ...
      const nonVegan = ['egg', 'eggs', 'meat', 'chicken', 'beef', 'fish', 'dairy', 'cheese'];
      likes = likes.split(/[,\s]+/)
        .filter(item => !nonVegan.some(nv => item.toLowerCase().includes(nv)))
        .join(', ');
...
const response = await openai.responses.create({
  ...
  max_output_tokens: 16000,
    ...
    console.log("Using simplified prompt for faster generation");
    ...
    // Use output_text FIRST (this is the complete generated text)
    if (response.output_text && response.output_text.trim()) {
      content = response.output_text.trim();
      console.log("Using output_text (length):", content.length);
    }

    // Fallback to output array
    else if (response.output && Array.isArray(response.output)) {
      console.log("Falling back to output array extraction");
        ...
        if (outputItem.type === "message" && outputItem.content) {
          for (const contentItem of outputItem.content) {
            if (contentItem.type === "text" && contentItem.text) {
              ...
              console.log(
                "Extracted from output array (length):",
                content.length
              );
              break;
            }
  ...
  for (let i = 0; i < daysCount; i++) {
    ...
    days.push({
      ...
      meals: [{
        ...
        ingredients: [
          { name: meal.protein, quantity: inputs.portions || 2, unit: "servings" },
          ...meal.sides.map(side => ({ name: side, quantity: 1, unit: "cup" }))
        ],
        cookingTime: 30,
        instructions: [
...
export async function testOpenAIAPI() {
  try {
    ...
    const testResponse = await openai.responses.create({
      model: "gpt-5-nano",
      input: "Write a very short test response about food.",
      store: true,
    ...
    console.log("Output text:", testResponse.output_text);
    return { success: true, data: testResponse };
  } catch (error) {
...
export async function generateChatGPTRecipe(prompt, options = {}) {
  try {
    ...
    // GPT-5-nano has very limited parameters. Let's use only what's documented
    const response = await openai.responses.create({
      model: "gpt-5-nano",
      input: [
        {
...
NO other text. NO explanations. NO markdown. ONLY the JSON object.`
    ...
    if (response.output_text) {
      ...
      console.log("Falling back to output array");
      for (const outputItem of response.output) {
        if (outputItem.type === "message" && outputItem.content) {
          for (const contentItem of outputItem.content) {
            if (contentItem.type === "text" && contentItem.text) {
    ...
    console.log("Content after cleaning (first 150 chars):", content.substring(0, 150));
